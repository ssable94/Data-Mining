...
class decision_tree(object):
	...
	def build_decision_tree(self, points_index, validation):
		...
		p_mc = self.get_majorClass_and_count(points_index)[0]
		vp_mc = (self.get_majorClass_and_count(validation)[2])[p_mc]

		l_mc = self.get_majorClass_and_count(left_pt)[0]
		vl_mc = (self.get_majorClass_and_count(left_validation)[2])[l_mc]

		r_mc = self.get_majorClass_and_count(right_pt)[0]
		vr_mc = (self.get_majorClass_and_count(right_validation)[2])[r_mc]

		if vr_mc + vl_mc > vp_mc:
			flag = True
		else:
			flag = False
		...
		if flag:
			...
			node.feature = attr
			node.value = value
			node.left = self.build_decision_tree(left_pt, left_validation)
			node.right = self.build_decision_tree(right_pt, right_validation)
		else:
